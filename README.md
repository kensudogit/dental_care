...
> **`Unknown argument "page"` / 古い API スキーマ**
>
> Web は自動で旧 API 形式にフォールバックしますが、**api サービスを Redeploy** すると本番データ＋ページネーション＋ clinical 機能が使えます。
> Railway → **api** → Deployments → **Redeploy**。`/health` の `version` が `2.1.0-pagination` になれば成功です。
