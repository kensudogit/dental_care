...
> 2. 再デプロイ後 `/status` で `unifiedDeploy: true` と `goBinaryPresent: true` を確認
> 3. Root Directory が `frontend` でも **一体型 Dockerfile** が Go API を同梱します
> 4. Deploy ログに `[unified] API ready` があること
